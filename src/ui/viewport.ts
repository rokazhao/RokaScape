...
import { getMonster, getMonsterCombatLevel } from '../content/monsters';
import {
...
export class Viewport {
  ...
  private showDialogue(npcId: string): void {
    ...
    npc.dialogues.forEach((d, i) => {
      const btn = document.createElement('button');
      btn.innerHTML = `<span class="idx">${i + 1}</span><span>${d.label}</span>`;
      btn.addEventListener('click', () => {
        this.game.talkToNpc(npcId, i);
        this.dialogueEl.classList.remove('visible');
      });
