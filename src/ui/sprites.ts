...
const PLAYER_ROWS_STRIDE: string[] = PLAYER_ROWS.map((row, y) => {
  if (y < 18) return row;
  if (y < 23) return '.......gggg....gggg.......';
  if (y < 27) return '......kkkkk....kkkkk......';
  return '.....kkkkkk....kkkkkk.....';
});

function makePlayer(): SpriteSheet {
  const frames: [Pixel[][], Pixel[][]] = [
    outlineFrame(paintGrid(PLAYER_ROWS, PLAYER_PALETTE)),
    outlineFrame(paintGrid(PLAYER_ROWS_STRIDE, PLAYER_PALETTE)),
  ];
  return { w: 26, h: 28, frames };
}
