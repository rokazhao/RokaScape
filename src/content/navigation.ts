 ...
 */
const NAVIGATION: Record<string, NavigationDef> = {
  lumbridge: {
    zones: [
      {
...
function isInPolygon(point: NavPoint, polygon: NavPolygon): boolean {
  let inside = false;
...
export function isWalkable(areaId: string, point: NavPoint): boolean {
  ...
  const inZone = nav.zones?.some((zone) => isInPolygon(point, zone)) ?? false;
  const onPath = (nav.paths ?? []).some((path) => {
    for (let i = 1; i < path.points.length; i++) {
      if (distanceToSegment(point, path.points[i - 1]!, path.points[i]!) <= path.width / 2) return true;
    }
    return false;
  });
  if (!inZone && !onPath) return false;

  return !(nav.blockers?.some((blocker) => isInPolygon(point, blocker)) ?? false);
}
...
function isSegmentWalkable(areaId: string, from: NavPoint, to: NavPoint): boolean {
  ...
  for (let i = 0; i <= steps; i++) {
    const t = i / steps;
    if (!isWalkable(areaId, { x: from.x + (to.x - from.x) * t, y: from.y + (to.y - from.y) * t })) {
      return false;
    }
  }
  return true;
...
function getGrid(area: AreaDef): NavGrid {
  ...
  const cols = Math.floor(area.mapWidth / CELL) + 1;
  const rows = Math.floor(area.mapHeight / CELL) + 1;
  const walkable = new Uint8Array(cols * rows);
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      walkable[row * cols + col] = isWalkable(area.id, { x: col * CELL, y: row * CELL }) ? 1 : 0;
    }
  }
  const grid = { cols, rows, walkable };
